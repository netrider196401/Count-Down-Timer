...
"""
    ...
    def update_button_states(self):
        ...
        if self.timer.isActive():
            ...
            self.pause_button.setText("暂停" if self.current_language == "zh" else "Pause")
            self.pause_button.clicked.disconnect()
            self.pause_button.clicked.connect(self.pause_countdown)
        elif self.is_paused:
            # 计时器已暂停
            self.start_button.setEnabled(False)
            self.pause_button.setText("继续" if self.current_language == "zh" else "Resume")
            self.pause_button.clicked.disconnect()
            self.pause_button.clicked.connect(self.resume_countdown)
        else:
            # 计时器未运行且未暂停
    ...
    def resizeEvent(self, event):
        ...
        # 计算中间区域的大小
        # 考虑到顶部菜单栏和底部控件的高度，使用窗口高度的85%作为可用高度
        # 这样可以让字体尽量放大，充满中间空间
